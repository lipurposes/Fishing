...
namespace PathCreationEditor {
    ...
    public class PathEditor : Editor {
        ...
        void DrawVertexPathInspector () {
            ...
            if (data.showVertexPathOptions) {
                using (var check = new EditorGUI.ChangeCheckScope ()) {
                    ...
                    data.displayVertexPoints = GUILayout.Toggle (data.displayVertexPoints, new GUIContent ("Show Vertex Points"));
            ...
            // If a point has been selected
            if (vertexPointSelectIndex != -1) {
                EditorGUILayout.LabelField ("Selected Point:");
                using (new EditorGUI.IndentLevelScope ()) {
                    var currentPosition = creator.path.GetPoint(vertexPointSelectIndex);
                    EditorGUILayout.Vector3Field ("Position", currentPosition);
                }
            }
        ...
        void DrawVertexPoint (Vector3 handlePosition, int i)
        {
            if (vertexPointSelectIndex >= creator.path.NumPoints)
            {
                vertexPointSelectIndex = -1;
            }
            float handleSize =
            ...
            var cap = capFunctions[globalDisplaySettings.anchorShape];
            PathHandle.HandleInputType handleInputType;
            handlePosition = PathHandle.DrawVertexHandle(handlePosition, handleSize, cap, handleColours, out handleInputType, i, vertexPointSelectIndex);
            
            switch (handleInputType) {
                case PathHandle.HandleInputType.LMBPress:
                    if (vertexPointSelectIndex != i) {
                        vertexPointSelectIndex = i;
                        Repaint ();
                    }
                    break;
            }
        ...
        void DrawVertexPathSceneEditor () {
            ...
            if (data.displayVertexPoints) {
                for (int i = 0; i < creator.path.NumPoints; i++) {
                    DrawVertexPoint(creator.path.GetPoint (i), i);
                }
            }
