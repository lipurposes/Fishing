...
namespace PathCreationEditor
{
    public static class PathHandle
    {
        ...
        static Dictionary<int, int> hashToControlId;
        private static Dictionary<int, int> controlIdToHash;

        static PathHandle()
        {
            ids = new List<int>();
            idHash = new HashSet<int>();
            hashToControlId = new Dictionary<int, int>();
            controlIdToHash = new Dictionary<int, int>();
            dstMouseToDragPointStart = float.MaxValue;
        }

        public static Vector3 DrawVertexHandle(Vector3 position, float handleDiameter, Handles.CapFunction capFunc, HandleColours colours, out HandleInputType inputType, int handleIndex, int selectedIndex)
        {
            int id = GetVertexControlId(handleIndex);
            ...
            int vertexSelectHandleId = selectedIndex;
            // Handle input events
            switch (eventType)
            {
                case EventType.MouseDown:
                    if (Event.current.button == 0 && Event.current.modifiers != EventModifiers.Alt)
                    {
                        if (dstToHandle == 0 && dstToMouse < dstMouseToDragPointStart)
                        {
                            ...
                            vertexSelectHandleId = handleIndex;
            ...
            switch (eventType)
            {
                case EventType.Repaint:
                    ...
                    Handles.color = colours.defaultColour;

                    if (id == GUIUtility.hotControl)
                    {
                        Handles.color = colours.selectedColour;
                    }
                    else if (vertexSelectHandleId == handleIndex)
                    {
        ...
        static int GetVertexControlId(int handleIndex)
        {
            string hashString = string.Format("vertexPathHandle({0})", handleIndex);
            int hash = hashString.GetHashCode();
            if (hashToControlId.ContainsKey(hash))
            {
                return hashToControlId[hash];
            }
            else
            {
                int id = GUIUtility.GetControlID(hash, FocusType.Passive);
                if (controlIdToHash.ContainsKey(id))
                {
                    Debug.Log("***********id 重复");
                    hashToControlId.Remove(controlIdToHash[id]);
                }
                hashToControlId[hash] = id;
                controlIdToHash[id] = hash;
                return id;
            }
        }
    }
